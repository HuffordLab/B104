...
#source: https://github.com/schmitzlab/The-prevalence-evolution-and-chromatin-signatures-of-plant-regulatory-elements/blob/master/Identification_of_HighQuality_ACRs/01_calculate_density.sh

if [ $# -lt 5 ]; then
    echo "usage:sh *.sh <output_name> <bin_size> <step> <path_to_sample_file> <path_to_input_file>"
    echo "
...
do
 ...
 total_lines=$(cat $reads |wc -l)

 bedtools coverage -a ${name}_split_$n -b $reads -counts |awk '{print $1,$2,$3,$4,'$size'*$5/'$total_lines'/($3-$2)}' OFS="\t" > ${name}_split_$n.$i.bgx
 for((i=1;i<$a;i++))
 do
 	j=$(($i-1))
  reads=sample_rep$i.Tn5
  ...
  bedtools coverage -a ${name}_split_$n -b $reads -counts |awk '{print '$size'*$5/'$total_lines'/($3-$2)}' OFS="\t" > ${name}_split_$n.$i.bg
